'use strict';
/* Estensioni VaaardStar — impostazioni, versioni, stampa, aptica */
(function(root){
  const DEFAULTS={
  ...
  function printDocument(name,content,font){
    const html='<!DOCTYPE html><html><head><meta charset="utf-8"><title>'+
      root.VSLib.escapeHtml(name)+'</title><style>@page{margin:2cm}pre{font-family:'+
      (font||'monospace')+';font-size:12pt;line-height:1.2;white-space:pre-wrap;word-break:break-word;color:#000;background:#fff}</style></head><body><pre>'+
  ...
  root.VSF={
    ...
    load:async getMeta=>{ mergeCfg(await getMeta('appSettings')||{}); return CFG; },
